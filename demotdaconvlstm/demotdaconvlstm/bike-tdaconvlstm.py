...
def load_npy_data(filename, split):
    ...
    train = data[:split[0], :, :, :]
    val = data[split[0]:split[0] + split[1], :, :, :]
...
# (26304, 16, 16, 2) (17520, 16, 16, 2) (4416, 16, 16, 2) (4368, 16, 16, 2)
# 归一化数据处理
...
print(val_data.shape)  # 4416,2,16,16
...
def attention_2(hidden_list, last_output, batch_size):
    output_flatten = torch.reshape(last_output, (batch_size, -1))  # torch.Size([16, 1024])
    h0_h9_flatten = torch.reshape(hidden_list, (batch_size, 9, -1))  # torch.Size([16, 9, 1024])
...
class Model(nn.Module):
    def __init__(self):
        super(Model,self).__init__()  # 输入（16，10，2,16,16）--->(160,2,16,16)
        self.conv_relu = nn.Sequential(
            nn.Conv2d(in_channels=2,out_channels=8,kernel_size=3,stride=1,padding=1),
            # nn.BatchNorm2d(8),     # (160,8,16,16)
            nn.ReLU(),
            # nn.Dropout2d(),
            nn.Conv2d(8,16,3,1,1),    # (160,16,16,16)
            # nn.BatchNorm2d(16),
            nn.ReLU(),
            # nn.Dropout2d()
        )
        ...
        # self.convlstmcell2 = ConvLSTMCell(input_dim=64,hidden_dim=64,kernel_size=(3,3),bias=True)
                                            ...
                                            # 16,64,16,16
        self.deconv = nn.Sequential(nn.ConvTranspose2d(64,8,3,1,1),
                                    nn.ReLU(),
            # nn.ConvTranspose2d(in_channels=16,out_channels=8,
            # kernel_size=3,stride=1,padding=1),
            #                         # nn.BatchNorm2d(8),
            #                         nn.ReLU(),
                                    nn.ConvTranspose2d(8,2,3,1,1),
                                    # nn.BatchNorm2d(2),
                                    nn.ReLU())
    def forward(self, x):
        b = []
...
def save():
    ...
    def weights_init(m):
        for p in model.parameters():
            ...
            if classname.find('Conv') != -1:
                torch.nn.init.xavier_normal_(p.weight.data,gain=0.3)
                torch.nn.init.xavier_normal_(p.bias.data,gain=0.3)
    ...
    for epoch in range(300):
        ...
        # print(train_loss)
        # print(batch_y.shape)  # 16,1,2,16,16
        M = np.prod(batch_y.shape)*len(train_dataloader)
        # print(M)    # 16*1*2*16*16*1094 = 8962048
        rmse=np.sqrt(train_loss/M)
        ...
        for i,(x,y) in enumerate(val_dataloader):
            ...
            val_loss+=loss.item()
        # print(y.shape)  # 16,1,2,16,16
        T_m = np.prod(y.shape)*len(val_dataloader)
        # print(T_m)  # 2252800
        test_rmse = np.sqrt(val_loss/T_m)
        print('epoch:', (epoch+1),"train_real_rmse",normlization.real_loss(rmse),"test_real_rmse", normlization.real_loss(val_rmse))

        torch.save(model.state_dict(), 'citybike-epoch'+str(epoch+1)+'model_param.pkl')
...
def restore_param():
    model = Model()
    for epoch in range(59,60,1):
        model.load_state_dict(torch.load('citybike-epoch'+str(epoch+1)+'model_param.pkl'))

        loss_func = nn.MSELoss(reduction='sum')
        model = model.cuda()
        # train_loss = 0
        # for i, (train_batch_x, train_batch_y) in enumerate(train_dataloader):
        #     train_batch_x = train_batch_x.cuda()
        #     train_batch_y = train_batch_y.cuda()
        #     out = model(train_batch_x)
        #     loss = loss_func(out, train_batch_y)
        #     train_loss += loss.item()
        #     rmse = np.sqrt(train_loss / (np.prod(train_batch_y.shape) * len(train_dataloader)))
        #
        #     # model.eval()
        # val_loss = 0
        ...
        #     loss = loss_func(output, val_batch_y)
        #     val_loss += loss.item()
        # val_rmse = np.sqrt(val_loss / (np.prod(val_batch_y.shape) * len(val_dataloader)))
        ...
        for i,(test_batch_x,test_batch_y) in enumerate(test_dataloader):
            ...
            test_loss+=loss.item()
        if not os.path.exists('testtdaconvlstm'):
            os.mkdir('testtdaconvlstm')
        np.save('testtdaconvlstm/prediction',prediction)
        np.save('testtdaconvlstm/target',target)

        test_rmse = np.sqrt(test_loss/(np.prod(test_batch_y.shape)*len(test_dataloader)))
        print('epoch',(epoch+1),'test_real_rmse',normlization.real_loss(test_rmse))
