...
class Spatial_Attention_layer(nn.Block):
    ...
    def forward(self, x):
        ...
        self.V_s.shape = (num_of_vertices, num_of_vertices)   #[N,N]
...
class cheb_conv_with_SAt(nn.Block):
    ...
    def forward(self, x, spatial_attention):
        ...
        for time_step in range(num_of_timesteps):
            ...
            graph_signal = x[:, :, :, time_step]  #[B,N,C_in]
            ...
            for k in range(self.K):
                ...
                # shape is (batch_size, V, F)
                #T_k_with_at:结合空间注意力的矩阵
                rhs = nd.batch_dot(T_k_with_at.transpose((0, 2, 1)),  #[B,N,N] batch_dot [B,N,C] --> [B,N,C]
                                   graph_signal)   #graph_signal:输入x
    ...
    def forward(self, x):
        ...
        x_TAt = nd.batch_dot(x.reshape(batch_size, -1, num_of_timesteps),
                  ...
                  .reshape(batch_size, num_of_vertices,
                           num_of_features, num_of_timesteps)  #[B,N,C,T]

        # cheb gcn with spatial attention
        spatial_At = self.SAt(x_TAt)  #空间注意力  Spatial attention
        spatial_gcn = self.cheb_conv_SAt(x, spatial_At)  #spatial_At :spatial attention
